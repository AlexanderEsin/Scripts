...
final_bs_dir_name = "All_trees"
relabelled_dir_name = "Tips_relabelled_BS"
relabelled_nbs_dir_name = "Tips_relabelled_NBS"
key_dir_name = "Tip_keys"

final_bs_dir_path   <-   file.path(directory, final_bs_dir_name)
relabelled_dir_path <-  file.path(directory, relabelled_dir_name)
relabelled_nbs_dir_path <- file.path(directory, relabelled_nbs_dir_name)
key_dir_path  <-  file.path(directory, key_dir_name)
...
for (tree in input_trees) {
  ...
  if (mowgli_mp_root_switch == 1) {
    output_tree_bs_name   <-  file.path(relabelled_dir_path, paste0("Relabelled_mp_", name, ".txt"))
    output_tree_nbs_name  <-  file.path(relabelled_nbs_dir_path, paste0("Relabelled_mp_", name, ".txt"))
  } else {
    output_tree_name  <-  file.path(relabelled_dir_path, paste0("Relabelled_unr_", name, ".txt"))
  }
  

  output_key_name     <- file.path(key_dir_path, paste0("TIP_KEY_", name, ".tsv"))
  output_key = ""
