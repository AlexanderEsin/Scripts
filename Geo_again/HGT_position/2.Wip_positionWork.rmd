...
## // Circular plots // ##
basicPositionPlot	<- function(dataDensityA, dataDensityB = NA, bgDensity, shrink = 1.4, titleCex = 1.2, titleName = "", tcl.offset = 1, uin = 2.1, bg = "#333233") {
...
# master_dir			<- "C:/Users/Sasha/Desktop/Geo_again2"
...
allProtDB_path		<- file.path(master_dir, "All_prot_db")
...
byTypebySpeciesGC_data	<- lapply(dataTypes_withAge, function(dataType) {

	message(paste0("Calculating GC content profiles for: ", dataType))
	...
	bySpeciesGC_list	<- lapply(binomial_list, function(species) {
		...
		byBinGC_list	<- lapply(seq(1:binLength), function(genome_bin) {

			# Define bin position
			bin_start		<- (genome_bin - 1) / binLength
			bin_end			<- genome_bin / binLength

			# Decide the data we need for this dataType
			if (is.na(penalty)) {
...
GC_allGenes			<- byTypebySpeciesGC_data$All
GC_allGenes$Type	<- factor(GC_allGenes$Type, levels = dataTypes_withAge)

GC_VerVsHGT			<- bind_rows(list(byTypebySpeciesGC_data$Ver, byTypebySpeciesGC_data$lHGT))
...
## Re-assign ymin/ymax for the subdivision bars for the GC content scale

verVsHGT_subdivs	<- lapply(binomial_list, function(speciesName) {
	bySpecies	<- subset(GC_VerVsHGT, Species == speciesName)
	subDivisionKeyLinear_df$ymin	<- min(bySpecies$GC_content, na.rm = TRUE)
	subDivisionKeyLinear_df$ymax	<- max(bySpecies$GC_content, na.rm = TRUE)
...
GCbyBin_all_plot	<- ggplot(GCbyBin_all, aes(x = BinIndex, y = GC_content, color = Type)) +
	scale_x_continuous(name = "Bin along genome", limits = c(0, 100), breaks = seq(0, 100, 10), minor_breaks = NULL) +
	...
	geom_smooth(
		...
		se = FALSE) +
	# Overall smooth line (cross-species)
	# geom_smooth(aes(color = Type, fill = Type), size = 1.5, se = TRUE, span = 0.25) +
	scale_color_manual(values = allGenes_col, guide = guide_legend(title = "Gene Type")) +
	scale_fill_manual(values = alpha(allGenes_col, 0.7), guide = FALSE) +
	darkTheme +
	theme(
...
GC_VerVsHGT_plot	<- ggplot(GC_VerVsHGT, aes(x = BinIndex, y = GC_content, color = Type)) +
	...
	# Overall smooth line (cross-species)
	# geom_smooth(aes(color = Type, fill = Type), method = "loess", size = 1.5, se = TRUE, span = 0.2) +
	# scale_color_manual(values = verVsHGT_cols, guide = guide_legend(title = "Gene Type")) +
...
# Relabel the tree
AG_binomTime_tree2				<- AG_taxidTime_tree
AG_binomTime_tree2$tip.label	<- binomials_df$binomial

binomTree_phylo <- ggtree(tr = AG_binomTime_tree2, ladderize = FALSE, col =  "#D9D9D9", size = 1.5) +
...
binomTree_clado <- ggtree(tr = AG_binomTime_tree2, branch.length = "none", ladderize = FALSE, col =  "#D9D9D9", size = 1.5) +
	geom_tiplab(angle = -90, hjust = -0.05, col =  "#D9D9D9") +
...
treeAsDf	<- subset(fortify(AG_binomTime_tree2, ladderize = FALSE), isTip)
...
# ------------------------------------------------------------------------------------- #
## All genome GC
GC_all					<- byTypebySpeciesGC_data$All
...
quartz(width = 16, height = 8, bg = "#333233", type = "pdf", file = "/Users/aesin/Desktop/Geo_again/HGT_position/Figures/GC_content/GC_allBySpecies.pdf")
print(GC_all_plot)
dev.off()
...
quartz(width = 22, height = 6, bg = "#333233", type = "pdf", file = "/Users/aesin/Desktop/Geo_again/HGT_position/Figures/GC_content/GC_VervsHGTbySpecies.pdf")
...
### This is very likely due to secondary AG2AG transfers from other (recent) branches into these taxa
### This needs to be confirmed, eg:
# binomial <- "Geobacillus sp. WCH70"
# perTypeData$lHGT$`4`$allPosData[which(perTypeData$lHGT$`4`$allPosData$binomial == binomial & perTypeData$lHGT$`4`$allPosData$Subgroup == TRUE),]

GC_OldvsNew			<- bind_rows(list(byTypebySpeciesGC_data$Old, byTypebySpeciesGC_data$Recent))
GC_OldvsNew$Type	<- factor(GC_OldvsNew$Type, levels = dataTypes_withAge)
oldVsNew_cols		<- wes_palette("Darjeeling1")[4:5]

bySpecies_count		<- lapply(binomial_list, function(binomial) {
	byType_count	<- lapply(unique(GC_OldvsNew$Type), function(dataType) {
		max			<- max(with(GC_OldvsNew, GC_content[which(Species == binomial & Type == dataType)]))
		count		<- length(with(GC_OldvsNew, GC_content[which(Species == binomial & Type == dataType)]))
		return(data.frame(Type = dataType, Species = binomial, Count = count, MaxVal = max, stringsAsFactors = FALSE))
	})
...
quartz(width = 22, height = 6, bg = "#333233", type = "pdf", file = "/Users/aesin/Desktop/Geo_again/HGT_position/Figures/GC_content/GC_OldvsNewbySpecies.pdf")
...
bySpecies_meanGC_list	<- lapply(binomial_list, function(speciesName) {
	...
	label		<- ifelse (identical(corrMethod, "pearson"), paste("italic(r) == ", corVal, "~italic(p) == ", pVal), paste("italic(rs) == ", corVal, "~italic(p) == ", pVal))
