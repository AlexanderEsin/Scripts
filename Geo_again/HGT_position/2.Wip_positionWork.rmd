...
allProtDB_path		<- file.path(master_dir, "All_prot_db_new")
...
## Select just "M" data as an example
M_data	<- subset(byCOGDispersionSolo_df, COGcat == "M")
ggplot(M_data, aes(x = orthGroup, y = sdLocation)) + geom_point()

## Select "M" groups that has an sd (i.e. > 2 genes) of less than 0.05 (clustered closely)
Mnear_groups	<- M_data$orthGroup[which(M_data$sdLocation < 0.05)]
ggplot(subset(M_data, orthGroup %in% Mnear_groups), aes(x = orthGroup, y = meanLocation)) + geom_point()

## Test a group = 259
test_Mgroup	<- Mnear_groups[1]

## All the data corresponding to the orthologs in Group 259
test_Mdata_tbl		<- dbSendQuery(conn, 'SELECT * FROM t1 WHERE orthGroup = :orthGroup')
dbBind(test_Mdata_tbl, param = list(orthGroup = test_Mgroup))
test_Mdata_df	<- dbFetch(test_Mdata_tbl)
dbClearResult(test_Mdata_tbl)
