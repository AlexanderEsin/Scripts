...
pacman::p_load("ape", "phylobase", "circular", "polyclip", "RCircos", "stringr", "plyr", "dplyr", "reshape2", "gtable", "grid", "gridBase", "ggplot2", "ggdendro", "GGally", "ggpubr", "wesanderson", "ggrepel", "phytools", "zoo")


sourceScriptDir	<- "/Users/aesin/Documents/Scripts/Geo_again/HGT_position"
#sourceScriptDir	<- "/Users/Sasha/Documents/Scripts/Geo_again/HGT_position"
...
# master_dir			<- "/Users/Sasha/Desktop/Geo_again2"
...
compareCOGs_bySubdivision	<- function(bySuvdiv_dataType_A, bySuvdiv_dataType_B, subDivision_list, clusterBy = "A", cogMinCutOff = 50) {
	...
	chisqByCOG_list	<- lapply(minCutOffCOGs, function(COG) {
		...
		# perform chisq
		chisq.pval		<- chisq.test(x = matrixForChisq)$p.value
		return(data.frame(COG = COG, chisqPval = chisq.pval, stringsAsFactors = FALSE))
	})
...
png(width = 20, height = 10, units = "in", res = 300, bg = "#333233", filename = "/Users/Sasha/Desktop/Geo_again/HGT_position/Temp_figures/Compare_bySubdiv/lHGTvVer_clusterBylHGT.png")
pushViewport(viewport(layout = grid.layout(nrow = 2, ncol = 1, heights = c(0.5, 1.5))))
...
png(width = 20, height = 10, units = "in", res = 300, bg = "#333233", filename = "/Users/Sasha/Desktop/Geo_again/HGT_position/Temp_figures/Compare_bySubdiv/lHGT_OldvRecent_clusterBylHGT.png")
pushViewport(viewport(layout = grid.layout(nrow = 2, ncol = 1, heights = c(0.5, 1.5))))
...
## Subset outliers
outlierSpecies		<- subset(outlierID_rollmean, BinIndex == 50 & (rollMean > 0.25 | rollMean < -0.25))$Species
outlierLineTypes	<- c("dotted", "dashed", "twodash")
...
StrandBias_AllvsVer			<- bind_rows(list(byTypeStrandBias_list$All, byTypeStrandBias_list$Ver))
StrandBias_AllvsVer$Type	<- factor(StrandBias_AllvsVer$Type, levels = c("All", "Ver", "lHGT", "sHGT", "Old", "Recent"))

allVsVer_cols	<- wes_palette("Darjeeling1")[1:2]

StrandBias_AllvsVer_plot	<- ggplot(StrandBias_AllvsVer, aes(x = BinIndex, y = StrandBias, color = Type)) +
	...
	geom_smooth(data = subset(StrandBias_AllvsVer, Type == "All" & !(Species %in% outlierSpecies)), aes(group = Species), col = alpha(allVsVer_cols[1], 0.5), size = 0.2, se = FALSE) +
	geom_smooth(data = subset(StrandBias_AllvsVer, Type == "Ver" & !(Species %in% outlierSpecies)), aes(group = Species), col = alpha(allVsVer_cols[2], 0.5), size = 0.2, se = FALSE) +
	geom_smooth(data = subset(StrandBias_AllvsVer, Type == "All" & (Species %in% outlierSpecies)), aes(linetype = lineType), col = alpha(allVsVer_cols[1], 1), size = 0.3, se = FALSE) +
	geom_smooth(data = subset(StrandBias_AllvsVer, Type == "Ver" & (Species %in% outlierSpecies)), aes(linetype = lineType), col = alpha(allVsVer_cols[2], 1), size = 0.3, se = FALSE) +
	geom_smooth(aes(color = Type), method = "loess", size = 1.5, se = FALSE) +
...
StrandBias_VervsHGT			<- bind_rows(list(byTypeStrandBias_list$Ver, byTypeStrandBias_list$lHGT))
StrandBias_VervsHGT$Type	<- factor(StrandBias_VervsHGT$Type, levels = c("All", "Ver", "lHGT", "sHGT", "Old", "Recent"))

StrandBias_VervslHGT_plot	<- ggplot(StrandBias_VervsHGT, aes(x = BinIndex, y = StrandBias, color = Type)) +
	scale_x_continuous(name = "Bin along genome", limits = c(0, 100), breaks = seq(0, 100, 10), minor_breaks = NULL) +
	geom_hline(yintercept = 0, col = alpha("#D9D9D9", 0.75), size = 1) + 
	scale_y_continuous(name = "Enrichment for genes on same strand as DnaA") +
	geom_smooth(data = subset(StrandBias_VervsHGT, Type == "Ver"), aes(group = Species), color = alpha(wes_palette("Darjeeling1")[2], 0.5), size = 0.2, se = FALSE) +
	geom_smooth(data = subset(StrandBias_VervsHGT, Type == "lHGT"), aes(group = Species), color = alpha(wes_palette("Darjeeling1")[3], 0.5), size = 0.2, se = FALSE) +
	geom_smooth(aes(color = Type, fill = Type), method = "loess", size = 1.5, se = FALSE) +
	stat_density(
		geom = "line",
		position = "identity",
		data = subset(StrandBias_VervsHGT, Type == "Ver" & is.na(StrandBias)),
		aes(x = BinIndex, y = ..scaled../4),
		adjust = 0.25,
	...
	stat_density(
		geom = "line",
		position = "identity",
		data = subset(StrandBias_VervsHGT, Type == "lHGT" & is.na(StrandBias)),
		aes(x = BinIndex, y = -..scaled../4),
		adjust = 0.25,
		...
		size = 0.5,
		inherit.aes = FALSE) +
	scale_color_manual(values = alpha(wes_palette("Darjeeling1")[2:4])) +
	scale_fill_manual(values = alpha(wes_palette("Darjeeling1")[2:4], alpha = 0.3), guide = FALSE) +
	darkTheme +
	theme(
		legend.justification = c(1, 1),
		legend.position = c(0.9, 0.9),
...
perBinBias_ver		<- byTypeStrandBias_list$Ver[!is.na(byTypeStrandBias_list$Ver$StrandBias),]

perBinBias_Recent	<- byTypeStrandBias_list$Recent[!is.na(byTypeStrandBias_list$Recent$StrandBias),]

perBinBiasMean_ver		<- dcast(perBinBias_ver, BinIndex ~ Type, mean, value.var = "StrandBias")
perBinBiasMean_recent	<- dcast(perBinBias_Recent, BinIndex ~ Type, mean, value.var = "StrandBias")


StrandBias_OldvsNew			<- bind_rows(list(byTypeStrandBias_list$Old, byTypeStrandBias_list$Recent))
StrandBias_OldvsNew$Type	<- factor(StrandBias_OldvsNew$Type, levels = c("All", "Ver", "lHGT", "sHGT", "Old", "Recent"))

StrandBias_OldvsNew_plot	<- ggplot(StrandBias_OldvsNew, aes(x = BinIndex, y = StrandBias, color = Type)) +
	scale_x_continuous(name = "Bin along genome", limits = c(0, 100), breaks = seq(0, 100, 10), minor_breaks = NULL) +
	geom_hline(yintercept = 0, col = alpha("#D9D9D9", 0.75), size = 1) + 
	scale_y_continuous(name = "Enrichment for genes on same strand as DnaA") +
	# geom_smooth(data = subset(StrandBias_VervsHGT, Type == "Ver"), aes(group = Species), color = alpha(wes_palette("Darjeeling1")[2], 0.5), size = 0.2, se = FALSE) +
	geom_smooth(data = subset(StrandBias_OldvsNew, Type == "Recent"), aes(group = Species), color = alpha(wes_palette("Darjeeling1")[3], 0.5), size = 0.2, se = FALSE) +
	geom_smooth(aes(color = Type), method = "loess", size = 1.5, se = FALSE) +
	stat_density(
		geom = "line",
		position = "identity",
		data = subset(StrandBias_OldvsNew, Type == "Old" & is.na(StrandBias)),
		aes(x = BinIndex, y = ..scaled../4),
		adjust = 0.25,
		color = wes_palette("Darjeeling1")[2],
		linetype = "longdash",
	...
	stat_density(
		geom = "line",
		position = "identity",
		data = subset(StrandBias_OldvsNew, Type == "Recent" & is.na(StrandBias)),
		aes(x = BinIndex, y = -..scaled../4),
		adjust = 0.25,
		color = wes_palette("Darjeeling1")[3],
		linetype = "longdash",
...
x <- subset(StrandBias_OldvsNew, Type == "Recent" & !(is.na(StrandBias)), drop = FALSE)
ggplot(x, aes(x = BinIndex)) + geom_histogram(binwidth = 100)
