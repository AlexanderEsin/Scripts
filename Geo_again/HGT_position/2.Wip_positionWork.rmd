...
byBinomGC_list		<- lapply(binomial_list, function(binomial) {
	print(binomial)

	allGeneProtIDs	<- perTypeData$All$allPosData$protID[which(perTypeData$All$allPosData$binomial == binomial)]
	allGenelocTags	<- perTypeData$All$allPosData$locusTag[which(perTypeData$All$allPosData$binomial == binomial)]

	nuclSeq_tbl		<- dbSendQuery(conn, 'SELECT NuclSeq FROM t1 WHERE protID = :protIDs AND locus = :locTags AND binomial = :binomial')
	dbBind(nuclSeq_tbl, param = list(protIDs = allGeneProtIDs, locTags = allGenelocTags, binomial = rep(binomial, length(allGeneProtIDs))))
	...
	allGeneGC_cont		<- rowSums(alphabetFrequency(nuclSeq_stringSet, as.prob = TRUE)[,2:3])
	perBinomGC_df		<- data.frame(GC_content = allGeneGC_cont, Species = rep(binomial, length(allGeneGC_cont)), stringsAsFactors = FALSE)

	return(perBinomGC_df)

})

byBinomGC_df	<- bind_rows(byBinomGC_list)

# Set up cols
perSpecies_cols	<- colorRampPalette(wes_palette("Darjeeling1"))(length(binomial_list))

# Set up tree
AG_binomNoEdge_tree <- AG_binomTime_tree
AG_binomNoEdge_tree$edge.length <- NULL



byBinomGC_plot	<- ggplot(data = byBinomGC_df, aes(x = Species, y = GC_content, fill = Species)) +
...
nuclSeq_tbl		<- dbSendQuery(conn, 'SELECT NuclSeq FROM t1 WHERE protID = :protIDs AND locus = :locTags')
dbBind(nuclSeq_tbl, param = list(protIDs = testHGT_protIDs, locTags = testHGT_locTags))
nuclSeq_df		<- dbFetch(nuclSeq_tbl)
dbClearResult(nuclSeq_tbl)
...
## For lHGT genes - calculate overall GC content
GC_cont_lHGT		<- data.frame(HGT_GC = rowSums(alphabetFrequency(nuclSeq_stringSet, as.prob = TRUE)[,2:3]))

ggplot(data = GC_cont_lHGT, aes(x = "HGT", y = HGT_GC)) +
	geom_violin(col = "#D9D9D9", fill = wes_palette("Darjeeling1")[1]) +
	scale_y_continuous(limits = c(0, 1)) +
	darkTheme
