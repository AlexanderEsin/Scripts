...
invisible(lapply(penalty_list, function(penalty) {
	...
	# Produce plot
	position_plot	<- circularDensityPlot(dataDensityA = lHGTDensity, dataDensityB = vertDensity, bgDensity = allDensity, shrink = 0.7, tcl.offset = 0.8, titleName = paste0("lHGT Enrichment at Penalty = ", penalty, "\nGenes = ", numGenes))
	replayPlot(position_plot)
}))
...
invisible(lapply(list("lHGT", "sHGT"), function(dataType) {
	...
	# Produce plot
	position_plot	<- circularDensityPlot(dataDensityA = HGTDensity, dataDensityB = vertDensity, bgDensity = allDensity, shrink = 0.7, tcl.offset = 0.8, titleName = paste0(dataType, " Enrichment at Penalty = ", penalty, "\nGenes = ", numGenes))
	replayPlot(position_plot)
}))
...
invisible(lapply(list("lHGT", "Group", "Subgroup"), function(type) {
	...
	# Produce plot
	position_plot	<- circularDensityPlot(dataDensityA = HGTDensity, dataDensityB = vertDensity, bgDensity = allDensity, shrink = 0.7, tcl.offset = 0.8, titleCex = 1.8, titleName = paste0(type, " Enrichment at Penalty = ", penalty, "\nGenes = ", numGenes))
	replayPlot(position_plot)
}))
