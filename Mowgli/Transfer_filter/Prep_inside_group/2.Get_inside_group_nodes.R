...
## 2. Paths ##
mowgli_dir	<- "/Users/aesin/Desktop/Mowgli/"
output_dir	<- paste0(mowgli_dir, "Inside_group/")
tree_file	<- paste0(mowgli_dir, "Species_tree/Reconciled_species_tree/outputSpeciesTree.mpr")

## 3. Get list of tips. Read in species tree ##
# Read in the list of inside group tips
IG_tip_df	<- read.table(paste0(output_dir, "Inside_group_tips.txt"), header = FALSE, sep = "\n", stringsAsFactors = FALSE)
...
write.table(node_labels, file = paste0(output_dir, "Inside_group_nodes.txt"), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\n")
