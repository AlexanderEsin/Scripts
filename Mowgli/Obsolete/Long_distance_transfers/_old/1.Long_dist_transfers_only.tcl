...
set long_HGT_penalty_l [list 4 5 6]
set full_penalty_list	[list 3 4 5 6]
...
if {$all_scenarios == TRUE} {
	set input_path  $mowgli_dir/Consistent_HGT_Vertical/Consistent_Full/All_scenarios
	set output_path $mowgli_dir/Long_distance_HGT/All_scenarios
} else {
	set input_path  $mowgli_dir/Consistent_HGT_Vertical/Consistent_Full/Scenarios_1_2
	set output_path $mowgli_dir/Long_distance_HGT/Scenarios_1_2
}
...
foreach penalty $full_penalty_list {
...
set IG_nodes_l [split [string trim [openfile $mowgli_dir/Inside_group_nodes.txt]] \n]
...
foreach orig_penalty $long_HGT_penalty_l {
	...
	# We test top down 
	set test_penalty_l	[lrange $full_penalty_list 0 [lsearch $full_penalty_list $orig_penalty]]
	...
	set HGT_groups_l [split [string trim [openfile $input_path/true_HGT_intersect_$penalty_list_str\.tsv]] \n]
	...
	multiputs $cross_pen_log stdout		"\nOriginal num of HGT groups: [llength $HGT_groups_l]\
										...
										\n\nIndependent lHGT events: [llength $per_penalty_lHGT_out]\
										\nIndependent sHGT events: [llength $per_penalty_sHGT_out]"
}
