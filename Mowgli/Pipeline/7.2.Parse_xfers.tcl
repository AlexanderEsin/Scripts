...
set transfer_penalties [list 3 4 5 6]

foreach transfer_cost $transfer_penalties {
	...
	set global_hgt_table [list "Gene Family\tHGT Present\tIs AG root"]
	set OutAG_present	0
	set global_OutAG	0
	set scenario_2		0
	set scenario_3		0
	...
	foreach out_dir $output_directories {
		...
		foreach non_trans_AG_event $NonTrans_AG {
			...
			if {[dict get $parsed_parent type] eq "Trans"} {
				...
				if {$transfer_type eq "OutAG"} {
					...
					lappend OutAG_done	"$out_dir\t$transfer_counter\tOutAG\t$donor_edge\t$receiver_edge\t$receiver_genet_parent\t$receiver_genet_child\tNA\t\|\|\t$parent_event"
		...
		while {[llength $AG2AG_process] > 0} {
			...
			if {[IsEventRoot $trans_event] == TRUE} {
				...
				lappend AG2AG_done "$out_dir\t$transfer_counter\tAG2AG\t$edge_output\t$AG2AG_heritage\t\|\|\t$trans_event"
		...
		if {[llength $OutAG_done] != 0} {
			set HGT_present 1
			incr OutAG_present
			lappend global_hgt_table "$out_dir\t1\t$AG_root"
		} else {
			set HGT_present 0
			lappend global_hgt_table "$out_dir\t0\t$AG_root"
		}
		...
		set out [open $mowgli_output_dir/$out_dir/Parsed_events.tsv w]
