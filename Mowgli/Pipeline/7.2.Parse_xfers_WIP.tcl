...
foreach out_dir $output_directories {
	...
	set events_list [parse_mapping_file Fullmapping.mpr]
	if {$events_list == 0} {
	...
	## Collapse redundant speciation edges - i.e. an edge that had multiple duplication, then as a result, had multiple speciation events ##
	...
	## If we find a speciation event the product of which is the same edge as from an external transfer - we can summarise those to just the transfer ##
	multiputs $dir_log stdout "\nAttempting to assign transfers to the speciation/duplication events ..."
	puts "\n\n[join $events_from_outside \n]\n\n"
	...
	foreach external_event $events_from_outside {
		...
		if {[lsearch -glob $transfer_from_outside *\;$edge*] > -1} {			
			...
			set parsed_transfer [transfer_subevent_parse2 $transfer_final_subevent]

			set trans_type				[dict get $parsed_transfer trans_type]
			set donor_edge 				[dict get $parsed_transfer donor_edge]
			set receiver_edge 			[dict get $parsed_transfer receiver_edge]
			set gene_tree_xferred_child	[dict get $parsed_transfer gene_tree_xferred_child]
			...
			lappend HGT_table_out "HGT: $donor_edge\t$receiver_edge\t$gene_tree_xferred_child"
			incr scenario_1_hgt
			set scenario 1
			...
			set transfer_hit [lsearch -inline -glob $transfer_from_anoxygeo *\;$edge*]
			set transfer_final_subevent [get_final_subevent $transfer_hit]
			set parsed_transfer [transfer_subevent_parse2 $transfer_final_subevent]

			lappend ag2ag_event_l "AG2AG: [join [list \
				[dict get $parsed_transfer donor_edge] \
				[dict get $parsed_transfer receiver_edge] \
				[dict get $parsed_transfer gene_tree_xferred_child]
			] \t]"
			set events_from_outside [lremove $events_from_outside $external_event]
		}
	}
	...
	foreach external_transfer $transfer_from_outside {
		...
		set parsed_transfer [transfer_subevent_parse2 $final_subevent]

		lappend HGT_table_out [join [list \
				[dict get $parsed_transfer donor_edge] \
				[dict get $parsed_transfer receiver_edge] \
				[dict get $parsed_transfer gene_tree_xferred_child]
		] \t]
		...
		multiputs $dir_log stdout "External transfer from \([dict get $parsed_transfer donor_edge]\) to Anoxy/Geobacillus \([dict get $parsed_transfer receiver_edge]\)"
	}
	...
	# //////////////// #
	# I think we put the AG2AG events here #
