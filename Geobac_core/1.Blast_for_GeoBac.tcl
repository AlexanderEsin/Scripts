	...
	foreach db $dblist {
		...
		if {$g != $db} {
			...
			set timer_forward [time {
				catch {exec blastp -query $direct/$org/Input/$query_dir/$fold_no/$g -db $direct/$org/Input/$db_dir/$db.db -out $direct/$org/Out_all/$x\&$y.tsv -evalue 1e-10 -outfmt 7 -max_target_seqs 1 -max_hsps 1 -num_threads 6}
			}]
