...
cd $direct
file mkdir Stitched_core_genome
file mkdir Group_fastas_MSA
cd $direct/Group_fastas_key_ids
...
set reference_file [string trim $reference_file]
set out [open $direct/Group_fastas_key_ids/MSA_reference_file.tsv w]
...
foreach file $fastas {
	...
	catch {exec muscle -in $direct/Group_fastas_key_ids/$file -out $direct/Group_fastas_MSA/$fas}
	puts "MUSCLING: $muscle_counter / [llength $fastas]"
	incr muscle_counter
}

file copy -force Species_KEY.txt $direct/Group_fastas_MSA/
file copy -force Species_KEY.txt $direct/Stitched_core_genome/
file copy -force MSA_reference_file.tsv $direct/Group_fastas_MSA/
file copy -force MSA_reference_file.tsv $direct/Stitched_core_genome/
...
foreach id_number $id_numbers {
	...
	puts "$id_number === [string length $master_CDS]"
	linebreak $master_CDS
	append MSA_master "$res\n"
}
set MSA_master [string trim $MSA_master]

set out [open $direct/Stitched_core_genome/MSA_master.faa w]
puts $out $MSA_master
close $out

cd $direct/Stitched_core_genome

exec seqret MSA_master.faa MSA_master.fas -osformat phylip
