...
## NB this is the AE updated script with option for manual max_target_seqs input and cov_cutoff passed to velvet ##
use strict;
use Getopt::Long;
use Pod::Usage;
use File::Basename;
...
GetOptions ('reads|sra|database|db=s' => \$atram_db,
            ...
            'exp_coverage|expected_coverage=s' => \$exp_cov,
            'kmer=i' => \$kmer,
            'bitscore=i' => \$bitscore,
            'evalue=f' => \$evalue,
            'length=i' => \$contiglength,
            'max_memory|memory=i' => \$max_memory,
            # Allow the max_target_seqs value to be manually stipulated as an optional parameter (max target sequences found per blast against each database shard #
            'max_target_seqs=i' => \$max_target_seqs,
            # Allow a coverage cutoff option to be passed to velvet #
            'cov_cutoff=f' => \$cov_cutoff,
            # Allow a velvet_log destination to be passed down to the assembler to store the STDOUT #
            'velvet_log=s' => \$velvet_log,
...
if ($max_memory > 0) {
	...
	if ($max_target_millions < 0) {
		...
		# Added this IF loop. If the max_target_seqs value is default as on line 62 - then adjust it based on the max_target_millions value (dependent on memory). If it was manually provided as a parameter AND differs from the default, then do not override it from the manual input value #
		if ($max_target_seqs == 100000000) {
