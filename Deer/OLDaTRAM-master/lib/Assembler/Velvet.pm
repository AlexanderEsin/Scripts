...
##############################################################
my $velvet_log_file = "/users/aesin/desktop/Deer/Assembly/Temp/Velvet_log.txt";
##############################################################
...
sub assembler {
	...
	my ($kmer, $tempdir, $ins_length, $exp_cov, $cov_cutoff, $min_contig_len, $output_file) = 0;
	my $longreads = "";
	my $velvet_log = "";
	...
	if ((ref $params) =~ /HASH/) {
		...
		if (exists $params->{"cov_cutoff"}) {
			$cov_cutoff = $params->{"cov_cutoff"};
		}
		...
		if (exists $params->{"velvet_log"}) {
			$velvet_log = $params->{"velvet_log"};
		}
	...
	run_command (get_bin($binaries->{velvetg}), "$tempdir -ins_length $ins_length -exp_cov $exp_cov -cov_cutoff $cov_cutoff -min_contig_lgth $min_contig_len >> $velvet_log", 1);
