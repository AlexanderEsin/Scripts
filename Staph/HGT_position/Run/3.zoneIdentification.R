...
p_load(tidyverse, RSQLite, ggpubr, wesanderson, gridExtra, seqinr)
...
zoneBounds	<- data.frame(boundary = c(0.045, 0.120, 0.240, 0.435, 0.52, 0.790, 0.880, 1.0, 1.045, 1.120, 1.240))
...
zoneRange_df$zoneType	<- c("Ver", "HGT", "HGT", "Ver", "HGT", "Ver", "HGT", "HGT", "Ver", "HGT", "HGT")
...
###########################################################################
## FOR BACILLUS THERE IS INSUFFICIENT SYMMETRY FOR toORI and toTER plots ##
###########################################################################
...
toOriDensity_withZones_plot	<- ggplot(data = normal_df_ext, aes(x = relGeneStart, color = type)) +
	scale_x_continuous(
		expand = expand_scale(mult = c(0.025, 0.025)),
		name = "Normalized genome position",
		# limits = c(0, 1),
...
print(toOriDensity_withZones_plot)
