...
set coreTaxid_file	$genomes_dir/Genome_lists/coreToKeep.tsv
...
set group_core_content	{}
set total_orths			0
set total_core_content	0
...
foreach group $mcl_group_list {
	...
	## Write the number of GROUP (e.g. AG  /BAC) proteins in each group to a global list
	lappend group_core_content	[join [concat $group_index $group_core_prot] \t]
	set total_core_content		[expr $total_core_content + $group_core_prot]
...
## Write out the core prot content per group at each evalue
set out		[open $group_output/Core_prot_content.tsv w]
puts $out	[join $group_core_content \n]
close $out
...
multiputs stdout $log "Total number of core group prots:\t$total_core_content"
