...
set data_sets {all vert const long short}
#set data_sets {long}
...
set per_penalty_tip_dir		/Users/aesin/Desktop/Mowgli/Mowgli_outputs/Per_penalty_tips/AG2AG_incl
...
foreach penalty $penalty_list {
	...
	foreach set $data_sets {
		...
		if {$set eq "all"} {
			...
			set events [split [string trim [openfile [glob /Users/aesin/Desktop/Mowgli/Constant_events/$vertical_file_name]]] \n]
		} elseif {$set eq "const"}	{
			set const_HGT_file_name "HGT_const_t*$penalty\.tsv"
			set events [split [string trim [openfile [glob /Users/aesin/Desktop/Mowgli/Constant_events/$const_HGT_file_name]]] \n]
		} elseif {$set eq "long"}	{
			set long_dist_file_name "T$penalty\_full_lHGT_events.tsv"
			# First line is header #
			set events [lrange [split [string trim [openfile /Users/aesin/Desktop/Mowgli/Refined_events/Events/$long_dist_file_name]] \n] 1 end]
		} elseif {$set eq "short"}	{
			set short_dist_file_name "T$penalty\_full_sHGT_events.tsv"
			# First line is header #
			set events [lrange [split [string trim [openfile /Users/aesin/Desktop/Mowgli/Refined_events/Events/$short_dist_file_name]] \n] 1 end]
		}
		...
		foreach event $events {
			...
			if {$set eq "long" || $set eq "short" || $set eq "const"} {

				# For lHGT input data structure see example of input file
				if {$set eq "long" || $set eq "short"} {
		...
		puts $out_log "Genes mapped to assembled Geobacillus genomes: $assembled_geo_counter"
