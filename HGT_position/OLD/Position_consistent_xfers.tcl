...
set transfers_sets {long const}

foreach set $transfers_sets {
	...
	if {$set eq "const"} {
		...
		set xfers [lrange [split [string trim [openfile $HGT_intersect_file]] \n] 0 end]
		...
		set xfers [lrange [split [string trim [openfile $long_distance_file]] \n] 0 end]
	}
	...
	foreach transfer $xfers {
		...
		set species_w_xfer [split [lindex [split [lindex $entry_in_recpt_tbl 0] \t] end] " "]
		...
		puts "Transfer events sorted: $xfer_counter / [llength $xfers]"
		incr xfer_counter
	...
	puts "\n\n\#\#\#\#\#\# Set: $set \#\#\#\#\#\#"
	puts "Xfers processed: $xfer_counter"
...
# set consistent_xfers [lrange [split [string trim [openfile $direct/Consistent_HGT_Vertical/Consistent_Full/Scenarios_1_2/true_HGT_intersect_t3_t4_t5.tsv]] \n] 0 end]
...
# foreach transfer $consistent_xfers {
# 	set group $transfer
...
# 	puts "Transfer events sorted: $xfer_counter / [llength $consistent_xfers]"
# 	incr xfer_counter
# }
