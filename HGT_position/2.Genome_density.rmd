...
GetCOGData	<- function() {
	...
	full_cog_tbl$mob_element	<- apply(full_cog_tbl, 1, function(row) {
		# message(row)
		if (length(grep(search_string, row[12], ignore.case = TRUE)) > 0) {
			TRUE
		} else {
			FALSE
		}
...
FunctionalEnrichmentBetweenSets	<- function(set_1, set_2, mobile_exclude = FALSE) {

	prop_table_list	<- lapply(penalty_l, function(penalty) {
		penalty_name	<- paste0("T", penalty)

		if (!mobile_exclude) {
			set1_df	<- as.data.frame(table(unlist(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_1)]]$cross_set_data$COG)))[-1,]
			set2_df	<- as.data.frame(table(unlist(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_2)]]$cross_set_data$COG)))[-1,]
		} else {
			set1_df	<- as.data.frame(table(unlist(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_1)]]$cross_set_data$COG[per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_1)]]$cross_set_data$Mobile_element == FALSE])))[-1,]
			set2_df	<- as.data.frame(table(unlist(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_2)]]$cross_set_data$COG[per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_1)]]$cross_set_data$Mobile_element == FALSE])))[-1,]

			set1_df <- rbind(set1_df, data.frame(Var1 = "W", Freq = length(unlist(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_1)]]$cross_set_data$COG[per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_1)]]$cross_set_data$Mobile_element == TRUE]))))
			set2_df <- rbind(set2_df, data.frame(Var1 = "W", Freq = length(unlist(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_2)]]$cross_set_data$COG[per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_2)]]$cross_set_data$Mobile_element == TRUE]))))
	...
	return(merged_prop_df	<- Reduce(function(df1, df2) full_join(df1, df2), prop_table_list))
}
...
## 5. Functional enrichment {.tabset .tabset-fade}

### 1. Functional profile
```{r plot_VertLong_v_All_COG, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 10, fig.height = 6, dev = 'png'}
plot_vert_v_all	<- PlotRelativeEnrichmentBoxes(FunctionalEnrichmentBetweenSets("vert", "all"), "vert", "all")
plot_long_v_all	<- PlotRelativeEnrichmentBoxes(FunctionalEnrichmentBetweenSets("long", "all"), "long", "all")
plot(plot_vert_v_all)
plot(plot_long_v_all)
```

```{r plot_long_v_vert_COG_enrich, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 10, fig.height = 6, dev = 'png'}
plot_long_v_vert_COGenrich	<- PlotRelativeEnrichmentBoxes(FunctionalEnrichmentBetweenSets("long", "vert"), "long", "vert")
plot(plot_long_v_vert_COGenrich)
```

```{r change_in_prot_groups_nums, message = FALSE, warning = FALSE, cache = TRUE}
# Number of proteins & groups in: all, vertical, long, short
all_penalties_stats	<- lapply(penalty_l, function(penalty) {
	penalty_name	<- paste0("T", penalty)
	stats_list	<- lapply(c("all", "vert", "long", "short"), function(set) {
		prot_number	<- length(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set)]]$cross_set_data$Full_protID)
		grp_number	<- length(unique(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set)]]$cross_set_data$Group))
...
### 2. Mobile elements
```{r plot_mobile_elems_COG, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 4, fig.height = 4, dev = 'png'}
# What COGs do mobile elements belong to?
mobile_element_COGs	<- as.data.frame(table(unlist(per_penalty_combined_xsets$T3$all$cross_set_data$COG[per_penalty_combined_xsets$T3$all$cross_set_data$Mobile_element == TRUE])))
mobile_element_COGs$Fraction	<- mobile_element_COGs$Freq / sum(mobile_element_COGs$Freq)
names(mobile_element_COGs) <- c("COG", "Frequency", "Fraction")
mobile_element_COG_plot	<-	ggplot(mobile_element_COGs, aes(x = COG, y = Fraction, fill = COG)) +
							geom_bar(stat = "identity") +
							scale_y_continuous("Fraction of Mobile elements in COG") +
							ggtitle("Mobile elements per COG") +
							theme_bw() +
							theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14,face = "bold"), plot.title = element_text(size = 20, hjust = 0.5), legend.text = element_text(size = 16), legend.title = element_text(size = 18), legend.key.size = unit(1, "cm"))
```

```{r mobile_elems_change_with_other_sets, message = FALSE, warning = FALSE, cache = TRUE}
...
print(cor(total_num_stats_df$NumProteins[total_num_stats_df$Set == "long"], total_num_stats_df$Mobile_elements[total_num_stats_df$Set == "long"]))
# [1] 0.980108
print(cor(total_num_stats_df$NumProteins[total_num_stats_df$Set == "vert"], total_num_stats_df$Mobile_elements[total_num_stats_df$Set == "vert"]))
# [1] 0.9558743
```

```{r plot_mobile_elems_xpen_xset, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 4, fig.height = 4, dev = 'png'}
xpenalty_mobile_elem_num	<- across_pen_mobile_df[across_pen_mobile_df$Set %in% c("long", "vert", "short"), ]
xpenalty_mob_elem_num_plot	<-	ggplot(xpenalty_mobile_elem_num, aes(x=Penalty, y = Mobile_elements, fill = Set)) +
								...
								theme_bw() +
								theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14,face = "bold"), plot.title = element_text(size = 20, hjust = 0.5), legend.text = element_text(size = 16), legend.title = element_text(size = 18), legend.key.size = unit(1, "cm"), strip.text.x = element_text(size = 12))
...
# large increase in mobile elements in Vertical set between T4 and T5 (and decrease in long set) due to 3 genes being predicted 
...
cat(paste0("Groups that intersect between T4_long and T5 vert: ", paste(long_to_vert, collapse = " ")))

# However > 60 proteins belong to these 3 groups
number_of_proteins	<- nrow(per_penalty_combined_xsets$T5$all$cross_set_data[per_penalty_combined_xsets$T5$all$cross_set_data$Group %in% long_to_vert, ])
...
long_v_vert_mob_remove	<- PlotRelativeEnrichmentBoxes(FunctionalEnrichmentBetweenSets("long", "vert", mobile_exclude = TRUE), "long", "vert")
...
per_penalty_distances	<- lapply(penalty_l, function(penalty) {
	...
	per_set_distances	<- lapply(working_sets, function(set) {
		...
		set_groups	<- unique(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set)]]$cross_set_data$Group)
		per_group_distances	<- lapply(set_groups, function(group) {
			per_group_df	<- per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set)]]$cross_set_data[per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set)]]$cross_set_data$Group == group,]
			mob_el_state	<- all(per_group_df$Mobile_element)

			if (nrow(per_group_df) > 2) {
				group_av_dist	<- (mean(dist.circular(circular(per_group_df$Circular_start), method = "geodesic")))/(2*pi)
				return(data.frame(Group = group, Av_dist = as.numeric(group_av_dist), Mobile_element = mob_el_state, stringsAsFactors = FALSE))
			} else {
				return(NA)
			}
		})
		per_group_distances		<- per_group_distances[!is.na(per_group_distances)]
		per_group_distances_df	<- bind_rows(per_group_distances)
		per_group_distances_df	<- per_group_distances_df[order(per_group_distances_df$Av_dist),]
	})
	names(per_set_distances)	<- working_sets
	per_set_distances$long_mob	<- per_set_distances$long[per_set_distances$long$Mobile_element == TRUE,]
	df_list <- lapply(names(per_set_distances),function(set_name) as.data.frame(cbind(Set = set_name, per_set_distances[[set_name]])))
	df <- bind_rows(df_list)
	ggplot(df, aes(x = Set, y = Av_dist)) + geom_boxplot()
	return(0)
})

par(mfrow = c(2,2))
lapply(per_penalty_distances, function(plot) replayPlot(plot))



long_T4_dists	<- lapply(unique(per_penalty_combined_xsets$T4$long$cross_set_data$Group), function(group) {
	per_group_df	<- per_penalty_combined_xsets$T4$long$cross_set_data[per_penalty_combined_xsets$T4$long$cross_set_data$Group == group,]
	mob_el_state	<- all(per_group_df$Mobile_element)
