...
combined_species_data	<- function(species = species, penalty = penalty, bandwith = bandwith, window = window, subgroup_data = subgroup_data, cog_data = cog_data, set_names = c("all", "const", "long", "vert", "short")) {
	...
	cog_dfs	<- lapply(all_dfs, function(set) {
		...
		# ptm <- proc.time()
		# with_cog_l	<- apply(set, 1, function(row) {

		# 	cog_tbl_row	<- cog_data[.(row[2]), nomatch = 0L]

		# 	if (nrow(cog_tbl_row) == 0) {
		# 		new_cols	<- data.frame(COG = I(list("-")), NOG = I(list("-")), Prot_name = "-", Mobile_element = FALSE, stringsAsFactors = FALSE)
		# 	} else {
		# 		COGs		<- list(str_trim(unlist(str_split(cog_tbl_row$COG.cat, ","))))
		# 		NOGs		<- list(grep("@NOG", unlist(str_split(cog_tbl_row$OGs, ",")), value = TRUE))
				
		# 		prot_name	<- cog_tbl_row$eggNOG.annot
		# 		mob_element	<- cog_tbl_row$mob_element

		# 		new_cols	<- data.frame(COG = I(COGs), NOG = I(NOGs), Prot_name = prot_name, Mobile_element = mob_element, stringsAsFactors = FALSE)
		# 	}
		# 	return(new_cols)
		# })
		# proc.time() - ptm
		...
		# cog_cols <- bind_rows(with_cog_l)
		# return(data.frame(cbind(set, cog_cols)))
	})
