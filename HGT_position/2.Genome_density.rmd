...
```
# Packages, functions & global variables {.tabset .tabset-fade}
## Packages
...
# Fix the dist.circular function so it doesn't try to convert the values (already in 2pi radians) into radians (conversion.circular). Conversion.circular is broken as it can't actually read in the numeric values in the circular object (stupid), but our values are already in 2pi format
dist.circular <- function (x, method = "correlation", diag = FALSE, upper = FALSE) {
	# modulo = "2pi" removed from conversion.circular
    x <- conversion.circular(x, units = "radians", zero = 0, 
        rotation = "counter")
    attr(x, "class") <- attr(x, "circularp") <- NULL
    if (!is.na(pmatch(method, "correlation"))) 
        method <- "correlation"
    METHODS <- c("correlation", "angularseparation", "chord", 
        "geodesic")
    ...
    d <- .C("R_distance", x = as.double(x), nr = N, nc = ncol(x), 
        ...
        method = as.integer(method), NAOK = TRUE, PACKAGE = "circular")$d
    attr(d, "Size") <- N
...
assignInNamespace("dist.circular", dist.circular, ns = "circular")
```
...
collate_across_species_norm	<- function(set, species_l, group_list) {
	...
	per_group_positions		<- lapply(group_list, function(group) {
		group_species_pos	<- lapply(species_l, function(species) {
			ref_tbl	<- combined_norm_pos_data[[species]]$norm_dfs[[set]]
		...
		names(group_species_pos)	<- species_l
		...
		keystone_pos	<- cbind(Species = keystone_spe, present_group_species_pos[[keystone_spe]][1,])
...
# / Updated to only plot faraway events / #
circos_pairwise_data_list	<- function(group_pos_collated, solo_plot = FALSE, near_plot = TRUE, dist_cutoff = 50000) {

	output_list	<- lapply(group_pos_collated, function(group) {
		...
		pairwise_per_group	<- lapply(seq_along(group$pos_data), function(pos_tbl_index) {
			# / If this is the first set of coords in this group and we don't want to solo plot,
			# we remove them because it would be identical to the key position. If we solo plot,
			# we link the key to itself to see the position of individual genes / #
			...
			if(nrow(pos_tbl) > 0) {
				pairwise_per_spec	<- lapply(1:nrow(pos_tbl), function(row_index) {
					# / If we want to plot the links between all genes (near_plot = T), then return
					# the relevant rows. If we want to plot links between faraway genes, then check
					# the distance between the key gene and other genes to decide inclusion / #
					if (near_plot == TRUE) {
						...
						key_start	<- key_position$START
						other_start	<- pos_tbl[row_index, 1]
						if (abs(key_start - other_start) > dist_cutoff) {
							return(data.frame(Chromosome = 1, chromStart = key_position$START, chromEND = key_position$END, Chromosome.1 = 1, chromStart.1 = pos_tbl[row_index, 1], chromEnd.1 = pos_tbl[row_index, 2]))
						} else {
							return(NULL)
						}
...
average_distance2ori	<- function(hgt_penalty = "T5", ver_penalty = NULL, set = "long") {
	...
	genes	<- per_penalty_combined_xsets[[as.character(penalty)]][[as.character(set)]]$cross_set_data[,c("Full_protID", "Group", "Rel_start", "Circular_start")]
	genes$Rel_start	<- as.numeric(genes$Rel_start)

	distance_2ori_l	<- apply(genes, 1, function(gene) {
		start <- as.numeric(gene[3])
	...
	distance_2ori_df	<- data.frame(t(distance_2ori_l), stringsAsFactors = FALSE)
	names(distance_2ori_df)	<- c("Full_proteinID", "Group", "Rel_start", "Circular_start", "Mod_dist")
...
FunctionalEnrichmentBetweenSets	<- function(set_1, set_2, mobile_seperate = FALSE) {

	prop_table_list	<- lapply(penalty_l, function(penalty) {
		penalty_name	<- paste0("T", penalty)

		set1_data	<- per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_1)]]$cross_set_data
		set2_data	<- per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set_2)]]$cross_set_data

		if (!mobile_seperate) {
			set1_df	<- as.data.frame(table(str_trim(unlist(set1_data$COG))))
			set2_df	<- as.data.frame(table(str_trim(unlist(set2_data$COG))))
		} else {
			set1_df	<- as.data.frame(table(str_trim(unlist(set1_data$COG[set1_data$Mobile_element == FALSE]))))
			set2_df	<- as.data.frame(table(str_trim(unlist(set2_data$COG[set2_data$Mobile_element == FALSE]))))

			set1_df <- rbind(set1_df, data.frame(Var1 = "W", Freq = length(unlist(set1_data$COG[set1_data$Mobile_element == TRUE]))))
			set2_df <- rbind(set2_df, data.frame(Var1 = "W", Freq = length(unlist(set2_data$COG[set2_data$Mobile_element == TRUE]))))

			rownames(set1_df) <- NULL
			rownames(set2_df) <- NULL
		}

		table_l <- lapply(list(set1_df, set2_df), function(table) {
			names(table) <- c("COG", "Freq")
			table$Fraction	<- table$Freq / sum(table$Freq)
			return(table)
		})

		merged <- Reduce(function(df1, df2) inner_join(df1, df2, by = "COG"), table_l)

		prop_table	<- as.data.frame(bind_cols(COG = merged[,1], Prop = (merged[,3]/merged[,5])))
		names(prop_table)	<- c("COG", penalty_name)
		return(prop_table)
	})
	merged_prop_df	<- Reduce(function(df1, df2) inner_join(df1, df2, by = "COG"), prop_table_list)
}
...
# / Cumulative fraction plot for two vectors of data (as graphic for KS-test) / #
CumulFractPlot	<- function(set_1, set_2, names = c("set_1", "set_2"), x_label = "x_value", plot_title = "NA") {
	# / Order the data / #
	set_1.ord	<- sort(set_1)
	set_2.ord	<- sort(set_2)

	# / Count data / #
	n_set_1		<- sum(!is.na(set_1.ord))
	n_set_2		<- sum(!is.na(set_2.ord))

	# / Plot data / #
	plot(set_1.ord, (1:n_set_1)/n_set_1, type = "s", xlab = x_label, ylab = 'Cumulative Frequency', main = plot_title)
	lines(set_2.ord, (1:n_set_2)/n_set_2, type = "s", col = "red")
	legend("bottomright", legend = names, lty=c(1,1), lwd = c(2,2), col = c("black", "red"))
}


```

# HGT density and separation by subgroup {.tabset .tabset-fade}
## Per-species HGT density plots
...
per_penalty_data_list	<- lapply(penalty_l, function(penalty) {
	...
	per_species_data_list <- lapply(assembled_species_l, function(species) {
		message("\tSpecies: ", species)
		combined_species_data(species = species, penalty = penalty, bandwith = bandwith, window = window_size, subgroup_data = subgroup_data, cog_data = cog_data)
	})
...
per_penalty_combined_xsets	<- lapply(penalty_l, function (penalty) {
	...
	combined_across_sets	<- lapply(combine_sets, function (set)	{
		...
		across_species_data_df$Dist2ori	<- with(across_species_data_df, ifelse(Rel_start < 0.5, Rel_start, 1 - Rel_start))
...
# Location and movement of vertically inherited genes {.tabset .tabset-fade}
## Distribution of vertical genes {#sect2.1}
...
unique_vert_groups	<- unique(per_penalty_combined_xsets[[ver_name]]$vert$cross_set_data$Group)
...
vert_groups_all_assembled	<- lapply(unique_vert_groups, function(group) {
	one2one_orth <- unlist(lapply(assembled_species_l, function(species) {
		...
		presence <- per_penalty_data_list[[ver_name]][[species]]$all_dfs$vert$Group %in% group
...
vert_groups_all_assembled	<- bind_rows(vert_groups_all_assembled)
...
distance_ranges		<- lapply(vert_groups_one2one_orth, function(group) {
	circ_range	<- circular_range(per_penalty_combined_xsets[[ver_name]]$vert$cross_set_data$Rel_start[per_penalty_combined_xsets[[ver_name]]$vert$cross_set_data$Group == group])
	return(data.frame(Group = group, Range = circ_range))
})
distance_ranges_df	<- bind_rows(distance_ranges)


## / Plot all the vertical gene family links over 100kbp / ##
# collated	<- collate_across_species_norm(set = "vert", species_l = assembled_species_l, group_list = vert_groups_one2one_orth)
# pairwise	<- circos_pairwise_data_list(collated, solo_plot = FALSE, near_plot = FALSE, dist_cutoff = 100000)
# link		<- circos_pairwise_link(pairwise)

# circos_plot	<- circos_link_plot(link)
...
## Moving & unmoving gene families {#sect2.2}
...
## "Unmoving" gene families do not rearrange
...
# Vertical genes: moving {.tabset .tabset-fade}
...
```{r plot_low_dist_per_species, message = FALSE, warning = FALSE, cache = TRUE, fig.length = 6, fig.height = 5, dev = c('jpeg', 'png')}
...
```{r plot_low_dist_circos, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 5, fig.height = 5, out.width = '1000px', dev = c('jpeg', 'png')}
lowdist_link_plot	<- circos_link_plot(low_dist2ori_link)
```


## Quarter dist to origin
...
```{r plot_quart_dist_per_species, message = FALSE, warning = FALSE, cache = TRUE, fig.length = 6, fig.height = 5, dev = c('jpeg', 'png')}
...
```{r plot_quart_dist_circos, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 5, fig.height = 5, out.width = '1000px', dev = c('jpeg','png')}
quartdist_link_plot	<- circos_link_plot(quart_dist2ori_link)
```

## Other dist to origin
...
```{r plot_other_dist_per_species, message = FALSE, warning = FALSE, cache = TRUE, fig.length = 6, fig.height = 5, dev = c('jpeg','png')}
...
```{r plot_othert_dist_circos, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 5, fig.height = 5, out.width = '1000px', dev = c('jpeg','png')}
otherdist_link_plot	<- circos_link_plot(other_dist2ori_link)
```


## Rearrangement towards origin
...
```{r plot_other_near_ori, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 6, fig.height = 6, out.width = '1000px', dev = c('jpeg','png')}
...
```{r outlier_genome_other, message = FALSE, warning = FALSE, cache = TRUE, fig.length = 9, fig.height = 7, dev = c('jpeg', 'png')}
...
```{r plot_other_near_ori_outlier_rem, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 9, fig.height = 3, out.width = '1000px', dev = 'png'}
...
# Location and movement of HGT genes {.tabset .tabset-fade}
### HGTs: Long vs Short
#### Looking at the distribution / movement of HGT genes that are consistently predicted at penalty =  `r HGT_penalty`
```{r long_and_short_data, message = FALSE, warning = FALSE, cache = TRUE}

# / Define a shortcut for the data set / #
x_species_HGT_data <- per_penalty_combined_xsets[[HGT_name]]

# / For each set of interest, identify unique groups and
# prepara the data for Circos plotting / #
hgt_sets	<- list("long", "short", "long_group", "long_subgroup")

hgt_link_data	<- lapply(hgt_sets, function(set) {
	unique_set_groups	<- unique(x_species_HGT_data[[set]]$cross_set_data$Group)
...
# / Rename the list to match the set names / #
names(hgt_link_data)	<- hgt_sets
```

```{r plot_long_short_circos, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 8, fig.height = 4, dev = c('jpeg','png')}
par(mfrow = c(1,2))
# / Plot the lHGT vs sHGT comparison / #
...
### lHGTS: Group vs Subgroup
```{r plot_group_subgroup_circos, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 8, fig.height = 4, dev = c('jpeg','png')}
par(mfrow = c(1,2))
# / Plot the lGrpHGT vs lSubHGT comparison / #
...
# Functional enrichment {.tabset .tabset-fade}
## COG Functional Profiles
### COG Enrichment: Versus "All"
#### Do the Vertical (vert) and lHGT (long) predicted gene sets have particular patterns of COG functional enrichment? We might expect highly conserved, well networked genes (e.g. Cell Motility - N) to be enriched in the Vertical gene set and depleted in the lHGT set relative to the total set (all) of genes. Similarly, Defence (V) genes may be enriched in the lHGT set
```{r plot_VertLong_v_All_COG, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 10, fig.height = 6, dev = 'png'}

# / Calculate the per-penalty enrichment between vert / long and the whole (all) set / #
VertVsAll_COG_enrich	<- FunctionalEnrichmentBetweenSets("vert", "all")
LongVsAll_COG_enrich	<- FunctionalEnrichmentBetweenSets("long", "all")

# / Plot the enrinchment profiles per COG. The plot is done on a log-scale, so 0 = no enrichment / #
VertVsAll_plot		<- PlotRelativeEnrichmentBoxes(VertVsAll_COG_enrich, "vert", "all")
LongVsAll_plot		<- PlotRelativeEnrichmentBoxes(LongVsAll_COG_enrich, "long", "all")
...
### COG Enrichment: lHGT vs Vertical
#### Here we calculate lHGT enrichment over the vertical genes - the "core" genome. A clear picture appears, though interestingly "L" - Replication, Recombination, Repair appears enriched for lHGT, which is at odds with its information processing, conserved COG type. We revisit this in the next section.
```{r plot_long_v_vert_COG_enrich, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 10, fig.height = 6, dev = 'png'}
# / Repeat as above, but lHGT vs Vertical set enrinchment. / #
LongVsVert_COG_enrich	<- FunctionalEnrichmentBetweenSets("long", "vert")
LongVsVert_plot			<- PlotRelativeEnrichmentBoxes(LongVsVert_COG_enrich, "long", "vert")

plot(LongVsVert_plot)
```


### NEEDS TO BE REARRANGED
```{r change_in_prot_groups_nums, message = FALSE, warning = FALSE, cache = TRUE}

# / Calculate the number of proteins & groups in: "all", "vertical", "long", "short" sets
all_penalties_stats	<- lapply(penalty_l, function(penalty) {
	# / Define penalty name / #
	penalty_name	<- paste0("T", penalty)
	stats_list	<- lapply(c("all", "vert", "long", "short"), function(set) {
		# / Number of proteins / #
		prot_number	<- length(per_penalty_combined_xsets[[penalty_name]][[set]]$cross_set_data$Full_protID)
		# / Number of groups / #
		grp_number	<- length(unique(per_penalty_combined_xsets[[penalty_name]][[set]]$cross_set_data$Group))
		# / Return as a data frame / #
		return(data.frame(Penalty = penalty_name, Set = set, NumProteins = prot_number, NumGroups = grp_number))
	})
	# / Bind per set data together into per-penalty data frames / #
	per_penalty_l <- bind_rows(stats_list)
})
# / Bind per-penalty data frames together into a single structure / #
across_penalty_df <- bind_rows(all_penalties_stats)
# / Order based on the set (e.g. "all") / #
across_penalty_df <- across_penalty_df[order(across_penalty_df$Set),]
print(across_penalty_df)
```

## Mobile elements
### Mobile elements: Which COGs?
#### We can determine that mobile elements across the whole dataset ("all" set) belong predominantly to the 'L' COG group
```{r plot_mobile_elems_COG, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 8, fig.height = 6, dev = 'png'}

# / Define the data in which we look for Mobile Element 
# gene families. In this case it's "all" genes. This
# set is identical across all penalties so T3 is fine. / #
all_mobile_element_data	<- per_penalty_combined_xsets$T3$all$cross_set_data

# / Dataframe of number of Mob. Elems. per COG / #
mob_elem_COG_df			<- data.frame(table(unlist(all_mobile_element_data$COG[which(all_mobile_element_data$Mobile_element == TRUE)])))

# / As a fraction of total Mob. Elems. / #
mob_elem_COG_df$Fract	<- mob_elem_COG_df$Freq / sum(mob_elem_COG_df$Freq)

# / Rename columns / #
names(mob_elem_COG_df)	<- c("COG", "Frequency", "Fraction")

# / Bar plot / #
mob_elem_COG_plot		<-	ggplot(mob_elem_COG_df, aes(x = COG, y = Fraction, fill = COG)) +
							geom_bar(stat = "identity") +
							scale_y_continuous("Fraction of Mobile Elements in COG") +
							ggtitle("Mobile Elements per COG") +
							theme_bw() +
							theme(axis.title = element_text(face = "bold"), plot.title = element_text(hjust = 0.5))
plot(mobile_element_COG_plot)
```

### Mobile Elements: Change with penalty
#### In this section we check determine how many mobile elements belong to each set, and how their prediction changes with penalty
```{r calc_mobile_element_xpen, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 8, fig.height = 8, dev = 'png'}
# / Number of mobile elements in all the sets across different penalties / #
across_pen_mobile_l <- lapply(c("all", "vert", "long", "short"), function(set) {

	set_mobile <- lapply(penalty_l, function(penalty) {
		penalty_name <- paste0("T", penalty)
		num_mob <- length(per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set)]]$cross_set_data$Full_protID[per_penalty_combined_xsets[[as.character(penalty_name)]][[as.character(set)]]$cross_set_data$Mobile_element == TRUE])
		return(data.frame(Penalty = penalty_name, Set = set, Mobile_elements = num_mob))
	})
	set_mobile_df	<- bind_rows(set_mobile)
})
across_pen_mobile_df	<- bind_rows(across_pen_mobile_l)
across_pen_mobile_df	<- across_pen_mobile_df[order(across_pen_mobile_df$Set),]

# / We combine the data so we can correlated the number of
# mobile elements predicted within each set with the number
# of genes predicted for each set. The correlations (pearson)
# are v. high / #
total_num_stats_df	<- merge(across_penalty_df, across_pen_mobile_df, by = c("Penalty", "Set"))
total_num_stats_df	<- total_num_stats_df[order(total_num_stats_df$Set),]

# / Correlation of number of Mob. Elements with number of long/vert proteins / #
temp <- cor(total_num_stats_df$NumProteins[total_num_stats_df$Set == "long"], total_num_stats_df$Mobile_elements[total_num_stats_df$Set == "long"])
# [1] 0.980108
temp <- cor(total_num_stats_df$NumProteins[total_num_stats_df$Set == "vert"], total_num_stats_df$Mobile_elements[total_num_stats_df$Set == "vert"])
# [1] 0.9558743

# / Select for which sets we want to plot Mob. Elem. data / #
xpenalty_mob_elem_num		<- across_pen_mobile_df[across_pen_mobile_df$Set %in% c("long", "vert", "short"), ]

# / Bar plot of Mob. Elem. predicted across penalties / #
xpenalty_mob_elem_num_plot	<- ggplot(xpenalty_mobile_elem_num, aes(x=Penalty, y = Mobile_elements, fill = Set)) +
								geom_bar(stat = "identity") +
								facet_wrap(~Set) +
								scale_y_continuous("Number of mobile elements") +
								ggtitle("Mobile element prediction across penalties/sets") +
								theme_bw() +
								theme(axis.title = element_text(face = "bold"), plot.title = element_text(hjust = 0.5))

plot(xpenalty_mob_elem_num_plot)
```

### Mobile Elements: Change with penalty (2)
#### Large increase in mobile elements in Vertical set between T4 and T5 (and decrease in long set) due to 3 gene families switching from long (T4) to vert (T5). These 3 groups are highly populated (62 total genes), leading to the jump we see in the graph above
```{r T4_T5_mob_elem_jump, message = FALSE, warning = FALSE, cache = TRUE}

# / Vertical groups at T5 and lHGT groups at T4 / #
vert_T5 <- unique(per_penalty_combined_xsets$T5$vert$cross_set_data$Group[per_penalty_combined_xsets$T5$vert$cross_set_data$Mobile_element == TRUE])
long_T4	<- unique(per_penalty_combined_xsets$T4$long$cross_set_data$Group[per_penalty_combined_xsets$T4$long$cross_set_data$Mobile_element == TRUE])

# / Intersect: groups that change from long (T4) to vert (T5)
# This doesn't consider groups not defined as long @ T4 that may 
# be predicted as T5 vert / #
long_to_vert	<- intersect(vert_T5, long_T4)

# / 3 groups intersect both sets/ #
cat(paste0("Groups that intersect between T4 (long) and T5 (vert): ", paste(long_to_vert, collapse = " ")))

# / However > 60 proteins belong to these 3 groups / #
number_of_proteins	<- length(per_penalty_combined_xsets$T5$all$cross_set_data$Full_protID[which(per_penalty_combined_xsets$T5$all$cross_set_data$Group %in% long_to_vert)])
cat(paste0("Total number of proteins belonging to these groups: ", number_of_proteins))
```

### COG Enrichment: Mobile elements
```{r plot_COG_enrich_mobile, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 10, fig.height = 6, dev = 'png'}
# / Seperate mobile elements into a seperate category - W / #
long_v_vert_COG_enrich_tab	<- FunctionalEnrichmentBetweenSets("long", "vert", mobile_seperate = TRUE)
long_v_vert_mob_remove	<- PlotRelativeEnrichmentBoxes(long_v_vert_COG_enrich_tab, "long", "vert")
plot(long_v_vert_mob_remove)
```

### Gene Dispersion: Calculation
#### Calculate per penalty & per set dispersion distances. I.e. measure the relative distribution of genes within each group - the aim, to see whether mobile elements (transposases) show similar or different dispersion to other HGT-derived genes. Expectation = higher mobility
```{r calc_dispersion_dists, message = FALSE, warning = FALSE, cache = TRUE}

per_penalty_distances	<- lapply(penalty_l, function(penalty) {

	# / Define penalty and names of sets / #
	penalty_name	<- paste0("T", penalty)
	working_sets	<- c("all", "vert", "long", "short")

	# / For each group within each set, get the average dispersion distance / #
	per_set_distances	<- lapply(working_sets, function(set) {

		# / Extract all unique group names from the dataset / #
		set_groups	<- unique(per_penalty_combined_xsets[[penalty_name]][[set]]$cross_set_data$Group)

		# / Keep track / #
		message(paste0(penalty_name, " : ", set))
		message(paste0("\tWorking on " ,length(set_groups), " groups..."))

		# / Using which to subset is 2-fold faster than subsetting by logical or using subset / #
		per_group_distances	<- lapply(set_groups, function(group) {

			# / Extract just the part of the df matching the group number / #
			per_group_df	<- per_penalty_combined_xsets[[penalty_name]][[set]]$cross_set_data[which(per_penalty_combined_xsets[[penalty_name]][[set]]$cross_set_data$Group == group),]
			# / Mobile element or not? / #	
			mob_el_state	<- all(per_group_df$Mobile_element)

			# / Only take the dispersion distance measurement if there is more than one gene / #
			if (nrow(per_group_df) > 1) {
				num_genes		<- nrow(per_group_df)
				group_disp_dist	<- mean(dist.circular(per_group_df$Circular_start, method = "geodesic")) / (2 * pi)
				return(data.frame(Group = group, Disp.Dist = group_disp_dist, Num.Genes = num_genes, Mobile.Element = mob_el_state, stringsAsFactors = FALSE))
			} else {
				return(NA)
			}
		})
		# / Remove any NA values - groups < 2 genes. Bind into DF; order by dispersion distance / #
		per_group_dists_no_na	<- per_group_distances[!is.na(per_group_distances)]
		per_group_distances_df	<- bind_rows(per_group_dists_no_na)
		per_group_distances_df	<- per_group_distances_df[order(per_group_distances_df$Disp.Dist),]
		# / Add a new column to properly scale the number of genes, with anything > 10 set as 10 / #
		per_group_distances_df$Gene.Scale	<- apply(per_group_distances_df, 1, function(row) ifelse(row[3] >= 10, 10, row[3]))
		# / Add a new column with a negative value for all mobile elements / #
		per_group_distances_df$Mobile.Value	<- apply(per_group_distances_df, 1, function(row) ifelse(row[4] == TRUE, -0.05, NA))
		return(per_group_distances_df)
	})
	# / Rename list based on set names / #
	names(per_set_distances)	<- working_sets
	return(per_set_distances)
})
# / Rename list based on penalty names / #
names(per_penalty_distances) <- lapply(penalty_l, function(pen) paste0("T", pen))
```


### Gene dispersion: Plotting
#### For each set calculate a plot combining all the dispersion data. Colored by the number of genes composing the group, and with the position of mobile elements indicated underneath
```{r plot_dispersion_dists, message = FALSE, warning = FALSE, cache = TRUE}
sets	<- c("all", "vert", "long", "short")
gene_dispersion_plots	<- lapply(sets, function(set) {
	penalty	<- "T4"
	plot_df	<- per_penalty_distances[[penalty]][[set]]

	plot_df$Group	<- factor(plot_df$Group, levels = plot_df$Group)

	plot	<- ggplot(plot_df, aes(x = Group, y = Disp.Dist, fill = Gene.Scale)) +
		geom_col(width = 1) +
	    scale_fill_distiller(palette="YlOrRd") +
		geom_point(aes(x = Group, y = Mobile.Value), size = 8, shape = 73, color = "black") +
	    theme(
...
names(gene_dispersion_plots)	<- sets
```

#### Dispersion distance plot for the "all" set
```{r plot_disp_dists1, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 12, fig.height = 4, dev = 'png'}
# / Plot the all gene distribution / #
gene_dispersion_plots$all
```

#### Dispersion distance plot for the "long" set
```{r plot_disp_dists2, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 12, fig.height = 4, dev = 'png'}
# / Plot the HGT (T4) gene distribution / #
gene_dispersion_plots$long
```



### Gene Dispersion: Testing
#### The gene dispersion amongst lHGT gene families is significantly different to gene dispersion in all genes. This is likely due to differences in the distribution of the numbers of genes in gene families - the "all" dataset contains the -core- vertical genes that typically have orthologs in every genome and are dispersed through Ori-Ter inversions given a particular dispersion profile. On the other hand, lHGT genes may have been acquired at any stage in lineage evolution (or multiple times independently) giving rise to pronounced extremes in dispersion as families containing 2 genes can have very large or low dispersion distances.

```{r dispersion_ttests1, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 8, fig.height = 6, dev = 'png'}
# / Tests of average gene dispersion of mobile elements in all and HGT sets / #
# / Results are consistent across T4 and T5 / #
# / Here: at T5 / #

penalty <- "T5"

# / The gene dispersion profiles between the "all" and "long" sets
#  are different. / # 
all_disp		<- per_penalty_distances[[penalty]]$all$Disp.Dist
long_disp		<- per_penalty_distances[[penalty]]$long$Disp.Dist

CumulFractPlot(all_disp, long_disp, names = c("All genes", "lHGT"), x_label = "Gene dispersion in gene family", plot_title = "Dispersion of genes in lHGT vs \"all\" gene families")
print(ks.test(all_disp, long_disp))

# / However, this is likely affected by the number of genes
# contained in the gene families. There is a clear difference
# in the gene content / family between "all" and "long" sets. / #
all_geneNum		<- per_penalty_distances[[penalty]]$all$Num.Genes
long_geneNum	<- per_penalty_distances[[penalty]]$long$Num.Genes

CumulFractPlot(all_geneNum, long_geneNum, names = c("All genes", "lHGT"), x_label = "Number of genes in gene family", plot_title = "Gene number in lHGT vs \"all\" gene families")
print(ks.test(all_geneNum, long_geneNum))
```

#### On the other hand, when comparing the distributions of gene number per family between lHGT genes and lHGT genes that are mobile elements, we find no difference. However, we still find a difference in the dispersion patterns of lHGT genes and lHGT mobile genes.
```{r dispersion_ttests2, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 8, fig.height = 6, dev = 'png'}
# / As might be expected, in terms of gene number, mobile elements
# are representative of the lHGT set. / #
long_mob_geneNum	<- per_penalty_distances[[penalty]]$long$Num.Genes[which(per_penalty_distances[[penalty]]$long$Mobile.Element == TRUE)]

CumulFractPlot(long_geneNum, long_mob_geneNum, names = c("lHGT (All)", "lHGT (Mobile)"), x_label = "Number of genes in gene family", plot_title = "Gene number in lHGT (non-mobile) vs lHGT (mobile) gene families")
print(ks.test(long_geneNum, long_mob_geneNum))

# / However, the dispersion profile is still significantly different. / #
long_mob_disp	<- per_penalty_distances[[penalty]]$long$Disp.Dist[which(per_penalty_distances[[penalty]]$long$Mobile.Element == TRUE)]

CumulFractPlot(long_disp, long_mob_disp, names = c("lHGT (All)", "lHGT (Mobile)"), x_label = "Gene dispersion in gene family", plot_title = "Gene dispersion in lHGT (non-mobile) vs lHGT (mobile) gene families")
print(ks.test(long_disp, long_mob_disp))
```


### Gene Dispersion: Sample mobile element (1643)
```{r plot_sample_transposase, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 4, fig.height = 4, out.width = '50%', out.height = '50%', dev = c('jpeg','png')}
# / Plot the positions of genes in a sample transposase gene family:
# Group = 1643 / #
group_1643		<- per_penalty_combined_xsets$T5$all$cross_set_data[which(per_penalty_combined_xsets$T5$all$cross_set_data$Group == 1643),]
collated_data	<- collate_across_species_norm(set = "long", group_list = list(1643), species_l = assembled_species_l)
pairwise_data	<- circos_pairwise_data_list(collated_data, solo_plot = TRUE)
link_df			<- circos_pairwise_link(pairwise_data)
circos_link_plot(link_df, title = paste0("Mobile elements: 1643"), linewidth = 1)
```


### Mobile Element: Overall Picture
#### The movement of mobile elements in the T4 and T5 lHGT sets
```{r circos_mobile_lHGT_T4_T5, message = FALSE, warning = FALSE, cache = TRUE, fig.width = 8, fig.height = 4, dev = c('jpeg', 'png')}

# / List of Mobile Element groups between T3 (most permissive) and T6 (conservative) - lHGT.
# Positional data is collated and linked ready for Circos plotting / #
lHGT_mobile_link_data	<- lapply(penalty_l, function(penalty) {
	# / Define the penalty name / #
	penalty_name	<- paste0("T", penalty)
	# / Shortcut to the dataset / #
	penalty_data	<- per_penalty_combined_xsets[[penalty_name]]$long$cross_set_data
	# / Unique groups in that penalty lHGT dataset that are identified as Mobile Elements / #
	unique_groups	<- unique(penalty_data$Group[which(penalty_data$Mobile_element == TRUE)])

	# / Prepare link data / #
	collated_data	<- collate_across_species_norm(set = "long", group_list = unique_groups, species_l = assembled_species_l)
	pairwise_data	<- circos_pairwise_data_list(collated_data, solo_plot = TRUE)
	link_df			<- circos_pairwise_link(pairwise_data)
...
# / Rename list based on penalty names / #
names(lHGT_mobile_link_data)	<- lapply(penalty_l, function(pen) paste0("T", pen))

# / Plot the T3 (most permissive) and T5 (semi conservative) link  plots / #
par(mfrow = c(1,2))
circos_link_plot(lHGT_mobile_link_data$T3, title = paste0("Mobile lHGT gene families at T3"), title_cex = 0.8, linewidth = 0.6)
circos_link_plot(lHGT_mobile_link_data$T5, title = paste0("Mobile lHGT gene families at T5"), title_cex = 0.8, linewidth = 0.6)
```


long_test_set	<- per_penalty_combined_xsets$T5$long$cross_set_data[which(per_penalty_combined_xsets$T5$long$cross_set_data$Group == 295),]

x <-	lapply(1:nrow(long_test_set), function(index) {
	row	<- long_test_set[index,]

	start_pos	<- row$Rel_start
	species		<- row$Species

	species_genes	<- per_penalty_data_list$T5[[species]]$all_dfs$all
	nearest_down	<- species_genes[which(species_genes$Rel_start == max(species_genes$Rel_start[species_genes$Rel_start < start_pos])),]
	nearest_up		<- species_genes[which(species_genes$Rel_start == min(species_genes$Rel_start[species_genes$Rel_start > start_pos])),]

	up_and_down	<- bind_rows(nearest_up, nearest_down)
...
group_1643		<- per_penalty_combined_xsets$T5$all$cross_set_data[which(per_penalty_combined_xsets$T5$all$cross_set_data$Group == 295),]
collated_data	<- collate_across_species_norm(set = "long", group_list = list(295), species_l = assembled_species_l)
pairwise_data	<- circos_pairwise_data_list(collated_data, solo_plot = TRUE)
link_df			<- circos_pairwise_link(pairwise_data)
circos_link_plot(link_df, title = paste0("Mobile elements: 295"), linewidth = 1)
...
# ## Density and position of mobile genes in the lHGT dataset ##
# bandwith = 2000
# bg_density			<- density.circular(per_penalty_combined_xsets$T5$all$circular_pos, kernel = "vonmises", bw = bandwith)
# lT5_mob_circ_pos	<- per_penalty_combined_xsets$T5$long$cross_set_data$Circular_start[which(per_penalty_combined_xsets$T5$long$cross_set_data$Mobile_element == TRUE)]
# lT5_mob_dens		<- density.circular(lT5_mob_circ_pos, kernel = "vonmises", bw = bandwith)
# all_plot			<- single_dens_pos_plot(lT5_mob_dens, bg_density, shrink = 1.7, name = "T5 lHGT mobile genes")



# T5_lHGT_groups	<- unique(per_penalty_combined_xsets$T5$long$cross_set_data$Group[which(per_penalty_combined_xsets$T5$long$cross_set_data$Mobile_element == TRUE)])
# collated_data	<- collate_across_species_norm(set = "all", group_list = T5_lHGT_groups, species_l = assembled_species_l)
# pairwise_data	<- circos_pairwise_data_list(collated_data, solo_plot = TRUE)
# link_df			<- circos_pairwise_link(pairwise_data)
# circos_link_plot(link_df, title = paste0("Mobile elements: T5 lHGT"), linewidth = 0.8)
...
# # 596 / 38027 mobile elements in ALL
# # 
...
# # 166 total mobile elements / 2934

# ### Vertical ###
# # 16668 total vertical proteins in dataset
# # 34 total mobile elements / 16668

# ### Short ###
# # 1341 total vertical proteins in dataset
# # 35 total mobile elements / 1341

# ### Long group ###
# # 2508 total vertical proteins in dataset
# # 131 total mobile elements / 2508 = 5%

# ### Long subgroup ###
# # 426 total vertical proteins in dataset
# # 35 total mobile elements / 426 = 8%

# ## Plot the position of all the mobile ele
# collated_data	<- collate_across_species_norm(set = "long", group_list = mobile_groups, species_l = assembled_species_l)
# pairwise_data	<- circos_pairwise_data_list(collated_data, solo_plot = TRUE)
# link_df			<- circos_pairwise_link(pairwise_data)

# circos_link_plot(link_df, title = paste0("Mobile element locations"), linewidth = 0.4)
...
# ## For each protein belonging to a set (e.g. long, vert, long_group) with a given distance to origin, assign it the necessary COGs and protein names
# # Takes a while
...
# ggplot(melt_vert_subdiv, aes(x = COG, y = value, fill = factor(variable))) + geom_bar(stat = "identity", position = "dodge") + labs(y = "Relative proportion of vertical genes close to Ori or Ter")
...
# # ## Read in the OG_annotations to determine transposase / integrase proteins ##
# # # Read in using fread (data.table) - much faster
# # OG_annotations	<- fread("/users/aesin/Desktop/Geo_analysis/Geo_ortholog_nucl/all_OG_annotations.tsv", sep = "\t")
# # OG_reduced_df	<- OG_annotations[,c(1,2,4)]
# # names(OG_reduced_df)	<- c("OG", "NOG_type", "Protein_name")

# # # Make an index on the OG column of this data table
# # setkey(OG_reduced_df, OG)
# # # Remove the raw read-in file
# # rm(OG_annotations)


# # # Read in all the updated COG functioncal annotations done for each protein in each group 09/2017
# # files	<- Sys.glob(file.path("/Users/aesin/Desktop/Geo_analysis/Geo_ortholog_nucl/Functional_annot/Output_annots", "*emapper.annotations"))
# # # We need to define each column, otherwise F COGs are read in as a logical
# # col_class	<- c("character", "character", "numeric", "numeric", "character", "character", "character", "character", "character", "character", "character", "character")

# # # A list containing the eggNOG annotation output for each group
# # group_tbls <- lapply(files, function(file) {
# # 	group_name	<- str_extract(file, "[0-9]+")
# # 	annot_data	<- read.table(file, header = T, sep = "\t", colClasses = col_class)
# # 	# The group number is added as a seperate column
# # 	annot_data$Group	<- rep(group_name, nrow(annot_data))
...
# # # Bind all together into one data table - we query this to assign NOGs to proteins in relevant sets below. Index this on the first column - protein names
# # full_cog_tbl <- bind_rows(group_tbls)
# # full_cog_tbl <- data.table(full_cog_tbl)
# # setkey(full_cog_tbl, query_name)




# # combined_cog_data2	<- lapply(1:length(distance_2ori_data), function(index) {

# # 	set			<- distance_2ori_data[[index]]
# # 	set_name	<- names(distance_2ori_data[index])
# # 	message(paste0("Working on : ", set_name))
	
# # 	HGT_cog_list <- apply(set, 1, function(entry) {

# # 		cog_tbl_row	<- full_cog_tbl[.(entry[1]), nomatch = 0L]

# # 		COGs	<- str_trim(unlist(str_split(cog_tbl_row$COG.cat, ",")))
# # 		NOGs	<- grep("@NOG", unlist(str_split(cog_tbl_row$OGs, ",")), value = TRUE)
# # 		mob_element	<- FALSE

# # 		if (length(NOGs) == 1) {
# # 			# nog_trimmed	<- unlist(str_split(NOGs, "@"))[1]
# # 			prot_name	<- cog_tbl_row$eggNOG.annot
# # 			# OG_tbl_row	<- OG_reduced_df[.(nog_trimmed), nomatch = 0L]
# # 			# prot_name	<- OG_tbl_row$Protein_name

# # 			# Check if the protein is a mobile element
# # 			if (length(grep(paste(c("transposase", "integrase", "transposon"), collapse = "|"), prot_name, ignore.case = TRUE)) > 0) {
# # 				mob_element <- TRUE
# # 			}
# # 		} else {
# # 			prot_name	<- "NA"
# # 		}
# # 		if (length(NOGs) == 0) NOGs <- "-"


# # 		if (length(COGs) > 0) {
# # 			per_cog_data	<- lapply(COGs, function(COG) {
# # 				return(data.frame(Full_protID = entry[1], Group = entry[2], Rel_start = entry[3], Circular_start = entry[4], Mod_dist = entry[5], COG = COG, NOG = NOGs, Prot_name = prot_name, Mobile_element = mob_element, stringsAsFactors = FALSE))
# # 			})
# # 			per_cog_df	<- do.call(bind_rows, per_cog_data)
# # 		} else {
# # 			per_cog_df	<- data.frame(Full_protID = entry[1], Group = entry[2], Rel_start = entry[3], Circular_start = entry[4], Mod_dist = entry[5], COG = "-", NOG = NOGs, Prot_name = prot_name, Mobile_element = mob_element, stringsAsFactors = FALSE)
# # 		}
# # 		return(per_cog_df)
...
# # })
# # names(combined_cog_data2)	<- names(distance_2ori_data)
