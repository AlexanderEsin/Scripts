...
###########################################################################
set penalty_list {4 5 6}
set highest_penalty [::tcl::mathfunc::max {*}$penalty_list]
set linear FALSE
...
###########################################################################
set log_dir /users/aesin/desktop/Geo_analysis/HGT_position/position_finder_log.log
puts stdout "Writing log out out to $log_dir"
set out_log [open $log_dir w]
...
set tip_key_dir $direct/Tip_keys_full

foreach penalty $penalty_list {

	
	set receptor_table_file $receptor_genomes_dir/Per_penalty_tips/Per_penalty_tips_t$tip_penalty\.tsv
	...
	foreach set $data_sets {
		...
		if {$linear == TRUE} {
			set output_dir /users/aesin/desktop/Geo_analysis/HGT_position/For_linear/T$penalty/Per_species_$set
		} else {
		...
		if {$set eq "const"} {
			...
			## 01.2017 changed the file that's matched with every HGT file ##
			cd $direct/Consistent_HGT_Vertical/Consistent_Full/Scenarios_1_2/
			set vertical_file [lindex [glob true_vertical_intersect_t[expr $penalty - 1]\*$highest_penalty\.tsv] 0]
		...
		foreach event $events {
			...
			if {$set eq "const" || $set eq "vert" || $set eq "all"} {
				...
				# Format of each event entry: '3643 3642	125 3484 3384'
			...
			foreach entry $entries_to_be_processed {
				...
				# 	set end_pos		[lindex $entry_l 6]
				# } else {
				# 	set start_pos	[lindex $entry_l 6]
				# 	set end_pos		[lindex $entry_l 5]
				# }
				set start_pos	[lindex $entry_l 5]
				set end_pos		[lindex $entry_l 6]
				...
				if {[regexp "Geobacillus" $species] == 1} {
					...
					set fraction_start_pos	[get_relative_position $start_pos $species_start $relevant_genome_length]
					set fraction_end_pos	[get_relative_position $end_pos $species_start $relevant_genome_length]
					...
					if {$set eq "long"} {
						...
						lappend $species\_start_tbl [join [concat $group $start_pos $receptor_node] " "]
						## Append the fractional position to the full entry table ##
						lappend $species\_tbl [join [concat $entry_l $fraction_start_pos $fraction_end_pos $receptor_node] " "]
						## Add fractional position to the per-species table ##
						lappend $species\_rel_start_tbl [join [concat $group  $fraction_start_pos $receptor_node] " "]
					} else {
						## Add raw position to the raw start table ##
						lappend $species\_start_tbl [join [concat $group $start_pos] " "]
						## Append the fractional position to the full entry table ##
						lappend $species\_tbl [join [concat $entry_l $fraction_start_pos $fraction_end_pos] " "]
						## Add fractional position to the per-species table ##
					...
					if {$linear == TRUE && $fraction_start_pos < 0.3} {
						set rollover_fraction_pos [expr $fraction_start_pos + 1]
		...
		if {$set eq "const"} {
			puts $out_log "Number of consistent transfer groups with > 1 transfer: $more_than_1_xfer_in_group"
		} elseif {$set eq "vert" || $set eq "all"} {
			puts $out_log "Total geo ids processed: $total_geo_ids"
		}
		puts $out_log "Genes we tried to map: $tried_to_map"
		puts $out_log "Genes that failed to map: $failed_to_map"
		puts $out_log "Genes mapping to plasmid: $plasmid_gene"
