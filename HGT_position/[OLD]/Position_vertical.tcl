...
if {$linear == TRUE} {
	set output_dir /users/aesin/desktop/Geo_analysis/HGT_position/For_linear/Per_species_vert
} else {
	set output_dir /users/aesin/desktop/Geo_analysis/HGT_position/For_circular/Per_species_vert
}
...
###########################################################################
## Get a list of the predicted vertical events ##
set vertical_xfers [lrange [split [string trim [openfile $direct/Consistent_HGT_Vertical/Consistent_Full/Scenarios_1_2/true_vertical_intersect_t5_t6_t8_t10_t20.tsv]] \n] 0 end]
...
set total_vert_ids 0

foreach group $vertical_xfers {
	
	set geo_ids [db1 eval {SELECT prot_id from t1 where group_number = $group AND binomial glob "Geobacillus*";}]
	set total_vert_ids [expr $total_vert_ids + [llength $geo_ids]]
	...
	incr group_counter
	puts "Vertical groups processed: $group_counter / [llength $vertical_xfers]"
}
...
foreach species $species_list {
	...
	set out [open $output_dir/Full_entries/$species\_vert.tsv w]
	puts $out $full_out_tbl
	close $out

	set out [open $output_dir/Start/$species\_vert.tsv w]
	puts $out $start_out_tbl
	close $out

	if {[regexp "Geobacillus" $species] == 1} {
		set rel_start_out_tbl [join [expr $$species\_rel_start_tbl] \n]
		set out [open $output_dir/Relative_start/$species\_vert.tsv w]
...
puts "\nGroups processed: $group_counter"
puts "Total geo ids process: $total_vert_ids"
