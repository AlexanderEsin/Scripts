...
foreach set $data_sets {
	...
	foreach event $events {
		...
		if {$set eq "long" || $set eq "const"} {
			...
			set total_vert_ids [expr $total_vert_ids + [llength $entries_to_be_processed]]
		}
	...
	if {$set eq "const"} {
		...
		puts "Total geo ids processed: $total_vert_ids"
	}
