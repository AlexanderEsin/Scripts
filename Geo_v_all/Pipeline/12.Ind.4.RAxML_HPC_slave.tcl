...
cd $direct/$split_dir/$group/$size
...
#########################################################################
cd $direct/$split_dir/$group/$size/$directory
...
## If we have a completed bootstrap file, then stitch the BS values onto the best ML tree ##
if {$BS_done_switch == 1} {
...
if {[llength [glob -nocomplain *bipartitions.*]] == 1} {
	set tree [lindex [glob -nocomplain *bipartitions.*] 0]
} elseif {[llength [glob -nocomplain *bestTree.*]] == 1} {
	set tree [lindex [glob -nocomplain *bestTree.*] 0]
}
