...
set fmap_path $direct/$ival

set output_path $direct/$ival/Missing_geobac_reconstruction
set output_dir_name Final_family_groups_all_geo
###
...
### Construct the sqlite databases for each of the e-value cutoff groups of families ###
if {$make_db_switch == 1} {
    foreach dir $eval_dirs {
        cd $direct/$ival/$dir
        set db_name Groups_$dir\_db
        ...
        db1 eval {create table t1(group_no int, genes blob)}

        openfile Group_list_$ival\.tsv
        set gene_families [split [string trim $data] \n]
        set x 1
        foreach family $gene_families {

            set family [string trim $family]
            set group_number [string trim [string range $family 0 [string first \t $family]]]
            set orthologs [string trim [string range $family [string first \t $family] end]]
            set orthologs [split $orthologs \t]

            foreach gene $orthologs {
                db1 eval {insert into t1 values($group_number,$gene)}
            }

            puts "$dir === $x / [llength $gene_families]"
            incr x

        }
        db1 eval {CREATE INDEX gene_index ON t1 (genes)}
