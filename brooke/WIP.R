...
crossSetOverlap_length_hmap <- recordPlot(heatmap.2(x = crossSetOverlap_length_matrix, Rowv = FALSE, symm = TRUE, dendrogram = "none", trace = "none"))
plot.new()
...
P1_codons_freq <- as.data.frame(trinucleotideFrequency(finalUniqSeq_data_list$P1$uniq_seq_data, with.labels = T, as.prob = T))
P4_codons_freq <- as.data.frame(trinucleotideFrequency(finalUniqSeq_data_list$P4$uniq_seq_data, with.labels = T, as.prob = T))
P8_codons_freq <- as.data.frame(trinucleotideFrequency(finalUniqSeq_data_list$P8$uniq_seq_data, with.labels = T, as.prob = T))

CodonsFreq_list <- list(P1_codons_freq, P4_codons_freq, P8_codons_freq)
x <- lapply(CodonsFreq_list, function(p_set) {
  codon_to_test <- "TTT"
  cols_to_test <- p_set[[codon_to_test]]
  return(cols_to_test)
})

P1_codons_df <- data.frame(Codon = names(P1_codons_freq), P1 = P1_codons_freq, row.names = NULL)
P4_codons_df <- data.frame(Codon = names(P4_codons_freq), P4 = P4_codons_freq, row.names = NULL)
P8_codons_df <- data.frame(Codon = names(P8_codons_freq), P8 = P8_codons_freq, row.names = NULL)

allCodonFreq_df <- Reduce(function(x, y) merge(x, y, by = "Codon"), list(P1_codons_df, P4_codons_df, P8_codons_df))
allCodonFreq_melt <- melt(allCodonFreq_df, id.vars = "Codon", variable.name = "PSet", value.name = "Frequency")
allCodonFreq_plot <- ggplot(allCodonFreq_melt, aes(x = PSet, y = Frequency)) + geom_bar(stat = "identity", position = "dodge") + facet_wrap(~Codon)
